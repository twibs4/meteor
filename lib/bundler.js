// xcxc release new dev bundle with the npm npm package
...
var PackageInstance = function (pkg, bundle) {
  ...
  self.api = {
    ...
    useNpm: function (npmDependencies) {
      ...
      // variant of npm.commands.install that plays well with Future.wrap
      var install2 = function(args, cb) {
        npm.commands.install(args, cb);
      };
      // variant of npm.commands.shrinkwrap that plays well with Future.wrap
      var shrinkwrap2 = function(cb) {
        npm.commands.shrinkwrap([], false/*silent*/, cb);
      };

      var npmDir = path.join(self.pkg.source_root, '.npm');
      if (fs.existsSync(npmDir)) {
        if (!fs.statSync(npmDir).isDirectory()) {
          throw new Error("Should be a directory: " + npmDir);
        }
        var shrinkwrapJsonPath = path.join(npmDir, 'npm-shrinkwrap.json');
        if (!fs.existsSync(shrinkwrapJsonPath)) {
          throw new Error("Expected to find: " + shrinkwrapJsonPath);
        }
        var packageJsonPath = path.join(npmDir, 'package.json');
        if (!fs.existsSync(packageJsonPath)) {
          throw new Error("Expected to find: " + packageJsonPath);
        }

        var installedDependencies = JSON.parse(fs.readFileSync(packageJsonPath)).dependencies;
        var dependenciesToUpdate = {};
        _.each(npmDependencies, function(version, name) {
          if (installedDependencies[name] !== npmDependencies[name]) {
            dependenciesToUpdate[name] = npmDependencies[name];
          }
        });

        if (!_.isEmpty(dependenciesToUpdate)) {
          var shrinkwrap = JSON.parse(fs.readFileSync(shrinkwrapJsonPath));
          _.each(dependenciesToUpdate, function(version, name) {
            delete shrinkwrap.dependencies[name];
          });
          fs.writeFileSync(shrinkwrapJsonPath, JSON.stringify(shrinkwrap));

          // XXX npm docs claim that one might be able to change npm.dir
          // rather than actually chdir but i couldn't get that to work
          // -avital
          var cwd = process.cwd();
          process.chdir(npmDir);
          Future.wrap(npm.load)().wait();

          /// xcxc <COPIED FROM BELOW>
          var updateStr = _.map(dependenciesToUpdate, function(version, name) {
            return name + '@' + version;
          }).join(', ');
          console.log('Updating npm dependencies for package ' + self.pkg.name + ': ' + updateStr);
          ...
          // xcxc MAKE SURE TO NOT CHOKE IF YOU HAVE NPM-SHRINKWRAP.JSON but not package.json

          _.each(dependenciesToUpdate, function(version, name) {
            Future.wrap(install2)([name + '@' + version]).wait(); // xcxc parallelize?
          });
          console.log('xcxc npm dir', npmDir);
          //          Future.wrap(shrinkwrap2)().wait();
          execSync("~/meteor/dev_bundle/bin/npm shrinkwrap"); // xcxc why can't we use line above this one?
          process.chdir(cwd);
        }
      } else {
        // xcxc why do we first see "Listening on" and only after finish bundling and installing npm?

        files.mkdir_p(npmDir);

        var updateStr = _.map(npmDependencies, function(version, name) {
          return name + '@' + version;
        }).join(', ');
        console.log('Updating npm dependencies for package ' + self.pkg.name + ': ' + updateStr);
        ...
        // package.json is redundant with the npm dependency declaration
        // within package.js so it shouldn't be version controlled. we
        // make sure to recreate it when first running the package
        fs.writeFileSync(path.join(npmDir, '.gitignore'), ['package.json', 'node_modules'].join('\n'));

        // execute the equivalent of: `cd $npmDir && npm install && npm shrinkwrap`
        ...
        Future.wrap(npm.load)().wait();
        npm.dir = npmDir;
        Future.wrap(install2)([]).wait();
        Future.wrap(shrinkwrap2)().wait();
        process.chdir(cwd);
      }

      var nodeModulesPath = path.join(npmDir, 'node_modules');
      var relNodeModulesPath = ['packages', self.pkg.name, 'node_modules'].join('/');
      // xcxc should we pass through something like add_resource?
