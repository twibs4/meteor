...
var Bundle = function () {
  ...
  // xcxc comment
  // these directories are copied without any intervention
  // maps path name (server relative) to path name (from which to read)
...
_.extend(Bundle.prototype, {
  ...
  write_to_directory: function (output_path, project_dir, dev_bundle_mode) {
    ...
    for (var rel_path in self.serverExternalDirs) {
      var path_in_bundle = path.join('app', rel_path);
      var full_path = path.join(build_path, path_in_bundle);
      console.log('xcxc cp_r', self.serverExternalDirs[rel_path], full_path);
