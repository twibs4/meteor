...
var PackageInstance = function (pkg, bundle) {
  ...
  self.api = {
    ...
    useNpm: function (npmDependencies) {
      ...
      var nodeModulesPaths = files.file_list_sync_full(path.join(npmDir, 'node_modules'));
      _.each(nodeModulesPaths, function(path) {
        var relPath = 'packages/' + self.pkg.name + /*with leading slash*/path.substr(npmDir.length);

        self.bundle.api.add_resource({
          type: "static", // xcxc really??
          path: relPath,
          source_file: path,
          where: 'server'
        });
...
_.extend(Bundle.prototype, {
  ...
  write_to_directory: function (output_path, project_dir, dev_bundle_mode) {
    ...
    for (var rel_path in self.files.server) {
      ...
      if (rel_path.indexOf('/node_modules/') === -1) { // xcxc better interface
