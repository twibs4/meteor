...
var PackageInstance = function (pkg, bundle) {
  ...
  self.api = {
    ...
    useNpm: function (npmDependencies) {
      ...
      // xcxc don't always recreate the .npm directory! instead diff
      // the contents' dependencies.json with `npmDependencies` and
      // upgrade appropriately:
      // 1. verify that npm-shrinkwrap.json exists
      // 2.1. if it is new, call `npm install foo@version` and update package.json
      // 2.2. if it was removed, `npm uninstall foo@version` and update package.json
      // 2.3. if its version changed, `npm install foo@newversion` and update package.json
      ...
      // xcxc run npm correctly... how? can we use the npm package?
      // https://npmjs.org/package/npm
      // xcxc make sure to deal with failures such as not having access to the network.
